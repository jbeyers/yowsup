...
class BroadcastLayer(YowInterfaceLayer):
    ...
    def onSuccess(self, successProtocolEntity):
        ...
        for target in self.getProp(self.__class__.PROP_MESSAGES, []):
            # getProp() is trying to retreive the list of (jid, message)
            # tuples, if none exist, use the default []
            phone, message = target
            print target
            messageEntity = BroadcastTextMessage(
                [Jid.normalize(i) for i in phone], message)
